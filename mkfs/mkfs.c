...
// inode 64 byte (must match kernel/include/fs/inode.h)
typedef struct inode_disk {
    ...
    unsigned int mtime;
    unsigned int ctime;
    unsigned int addrs[10];  // N_ADDRS = 10 (8 direct + 2 indirect)
} inode_disk_t;

// directory entry 34 byte (must match kernel/include/fs/dir.h)
typedef struct dirent {
    unsigned short inode_num;
    unsigned short hash;          // 名字的哈希值
    char name[30];
} dirent_t;
...
#define N_LOG_BLOCK      30
#define N_DATA_BLOCK     8192 // 1个block的bitmap管理的极限
#define N_INODE_BLOCK    128  // 支持2048个文件
#define N_BLOCK          (N_DATA_BLOCK + N_INODE_BLOCK + N_LOG_BLOCK + 3)  // 五个部分组合起来
...
// 与inode管理的data block相关 (must match kernel/include/fs/inode.h)
...
#define N_ADDRS_3 0   // 取消二级间接块
#define N_ADDRS (N_ADDRS_1 + N_ADDRS_2)  // = 10
...
// 计算字符串哈希值（必须与内核一致）
unsigned short hash_string(const char* s)
{
    unsigned short h = 0;
    while (*s) {
        h = (h << 5) + h + (unsigned char)*s++;  // h = h * 33 + c
    }
    return h;
}
...
{
    dirent_t de;
    de.inode_num = xshort(inode_num);  // ✅ 使用 xshort 而不是 xint
    de.hash = xshort(hash_string(name));  // ✅ 计算并设置哈希值
...
int main(int argc, char* argv[])
{
    ...
    sb.log_start = xint(1);                                    // 日志起始：块 1
    sb.log_blocks = xint(N_LOG_BLOCK);                        // 日志块数：30

    sb.inode_bitmap_start = xint(1 + N_LOG_BLOCK);            // inode位图
    sb.inode_start = xint(1 + N_LOG_BLOCK + 1);               // inode区
    sb.data_bitmap_start = xint(1 + N_LOG_BLOCK + 1 + N_INODE_BLOCK);  // 数据位图
    sb.data_start = xint(1 + N_LOG_BLOCK + 1 + N_INODE_BLOCK + 1);     // 数据区
